...
const CallResponse = ({ trail, saving, reload, handleTab }) => {
    ...
    const handleFollwDate = date => {
        setFeedback({ follow_up_date: 'valid' })
    ...
    const handleTimeConfim = date => {
        setFeedback({ follow_up_time: 'valid' })
    ...
    // },[])
    const isValidate = () => {
        if (visibleFields.includes('cal_desc')) {
            cal_desc === '' || cal_desc === undefined ? setFeedback({ cal_desc: 'err' }) : null
        }
        if ((visibleFields.includes('follow_up_time'))) {
            followTime === '' || followTime === undefined && setFeedback({ follow_up_time: 'err' })

        }
        if (visibleFields.includes('follow_up_date')) {
            followDate === '' || followDate === undefined && setFeedback({ follow_up_date: 'err' })
        }
    }
    console.log('followTimedee', feedback.follow_up_time);
                ...
                <ScrollView style={{ height: '60%' }}>
                    {call_response === 5 && spoke_calls_count == 0 ? (
                        <Space direction="column">
                            ...
                            <Space style={[styles.semiblock, feedback.follow_up_date === 'err' ? { backgroundColor: 'red' } : null]}>
                                <TextInput
                    ...
                    {visibleFields.includes('follow_up_date') ? (
                        <Space flexWrap={'wrap'}>
                            <Space direction="column">
                                ...
                                <Space style={[styles.semiblock, feedback.follow_up_date === 'err' && { borderColor: '#E92C2C' }]}>
                            ...
                            <Space direction="column">
                                ...
                                <Space
                                    style={[styles.semiblock, feedback.follow_up_time === 'err' && { borderColor: '#E92C2C' }]}
                    ...
                    {visibleFields.includes('cal_desc') ? (
                        <Space direction="column">
                            ...
                            <TextInput
                                style={[styles.textarea, feedback.cal_desc === 'err' && { borderColor: '#E92C2C' }]}
                                multiline={true}
                                numberOfLines={8}
                                onChangeText={(value) => {
                                    setFeedback({ cal_desc: 'valid' })
                                    setResponse({
                                        cal_desc: value,
                                    })
                                }
