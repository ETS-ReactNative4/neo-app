...
const CommonRate = ({foreignAmount, foreignCurrency, nativeAmount, nativeCurrency, componentStyle}) => {

  if(!componentStyle) componentStyle = {};

  return (
    <View style={[styles.container, componentStyle]}>
      <Text style={styles.font}>{foreignAmount} {foreignCurrency}</Text>
      <Text style={styles.font}>{nativeAmount} {nativeCurrency}</Text>
    </View>
  );
};
