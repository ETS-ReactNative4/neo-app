...
const NotifCard = ({
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.8}
      style={[styles.cardWrapper(cardWidth), {backgroundColor}]}
      onPress={action}
      onLayout={onLayout}>
        ...
        {cta ? (
          <View style={styles.messageLinkWrapper}>
            <TouchableOpacity activeOpacity={0.8} onPress={action}>
              <Text style={[styles.messageLink, {color: ctaColor}]}>{cta}</Text>
            </TouchableOpacity>
          </View>
        ) : null}
      </View>
...
const styles = StyleSheet.create({
  cardWrapper: cardWidth => ({
    width: responsiveWidth(100) - (cardWidth || 48),
    paddingVertical: 16,
    paddingHorizontal: 8,
    borderRadius: 5,
    flexDirection: 'row',
